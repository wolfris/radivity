// gendiscretesky.cpp : Defines the entry point for the console application.
//
...
int main(int argc, char* argv[])
{
	...
	const double groundRadiance=sky.GetAnnualIrradiance()*0.2/M_PI;
	...
	if (!(argc>1))
	{
		...
		fprintf(stderr,"gendiscretesky: Error - invalid input parameters\n");
		goto USAGEERROR;
	}
	...
	while (counter<argc-1)
	{
		...
		{
			fprintf(stderr,"gendiscretesky: Error - invalid input parameter '%s'\n\n",argv[counter]);
			goto USAGEERROR;
		}
	}
	...
	{
		fprintf(stderr,"gendiscretesky: Error reading climate file %s\n\n",filename);
		goto USAGEERROR;
	}
	...
	printf("{ This .cal file was generated automatically by gendiscretesky }\n");
	printf("{ ");
	for (j=0; j<argc; j++)
		printf("%s ",argv[j]);
	printf(" }\n\n");
...
USAGEERROR:
	fprintf(stderr,"Usage: gendiscretesky [-d] [+s1|+s2] [-a latitude] [-o longitude] [-l] [-m standard meridian] [-h hourshift] [-G|-B] <climate file>\n");
