...
bool cPerezSkyModel::SetSkyConditions(double Idh, double Ibh, cSun *Sun)
{
	...
	if (PerezClearness <0.999)
	{
		fprintf(stderr,"ERROR! CLEARNESS < 1 Clearness: %f Solar Zenith: %f, Idh: %f Ibh: %f\n",PerezClearness,SolarZenith,Idh,Ibh);
		return false;
	}
...
//	if (m_SolarAlt*180/M_PI + 3.885 >=0)
	...
	{
		// Solar altitude is very low so ignore this hour, warn user if Idh is relatively large (>10W/m2)

        if (Idh > 10)
		{
			fprintf(stderr,"Error!  Solar altitude < 6 degrees and Idh > 10 W/m^2 on day %d!  Ignoring day!\n",Sun->GetDay());
		}
