...
old_red(x) = clip(1.6*x - .6);
old_grn(x) = clip(if(x-.375, 1.6-1.6*x, 8/3*x));
old_blu(x) = clip(1 - 8/3*x);

mno_red(x) = 1.6*x - 0.6;
mno_grn(x) = if(x-.375, 1.6-1.6*x, 8/3*x);
mno_blu(x) = 1 - 8/3*x;

mono_b_red(x) = .04;
mono_b_grn(x) = if(x-.8, .04, 1.-1.2*x);
mono_b_blu(x) = if(x-.8, 2.44-1.8*x, 1.);
...
TEMPLATE_PC1 = """
norm : mult/(scale-min_scale)/le(1);
offset  : min_scale/(scale-min_scale);

v = map(li(1)*norm-offset);

vleft = map(li(1,-1,0)*norm-offset);
vright = map(li(1,1,0)*norm-offset);
vabove = map(li(1,0,1)*norm-offset);
vbelow = map(li(1,0,-1)*norm-offset);
...
#	    data = self._popenPipeCmd(cmd, stdin, f)
	    data = self._popenPipeCmd(cmd, stdin)
            f.write(data)
	    if os.name == 'nt':
	        path = path.replace("\\","\\\\")
	    return path
    ...
    def _createTmpDir(self):
        ...
        print >> sys.stderr, "\tTmpdir created:",self.tmpdir
	...
	cmd = str(cmd)
        ...
        pipe_tmpdir = tempfile.mkdtemp()
        fd,tmp_file=tempfile.mkstemp(suffix=".pic", dir=pipe_tmpdir)
        print >> sys.stderr, "Created temporary file to store pic:", tmp_file 
        f00 = open(tmp_file,'wb')
#        p = Popen(shlex.split(cmd), bufsize=-1, stdin=PIPE, stdout=data_out, stderr=PIPE)
        p = Popen(shlex.split(cmd), bufsize=-1, stdin=PIPE, stdout=f00.fileno(), stderr=PIPE)
        data_none, err = p.communicate(data_in)
        f00.close()
        f01 = open(tmp_file,'rb')
        data=f01.read()
        data_out=data
        f01.close()
        print >> sys.stderr, "Read temporary pic file into data." 
        if err:
    ...
    def createText(self):
        ...
        for i in range(self.steps):
            if self.decades > 0:
                ...
                value = ((self.scale-self.minimum) * (self.steps - self.zerooff - i) / self.steps) + self.minimum
            textlist.append(self.formatNumber(value))
        if self.zerooff == 0:
            textlist.append(self.formatNumber(self.minimum))
...
class FalsecolorImage(FalsecolorBase):
    ...
    def applyMask(self):
        ...
        print >> sys.stderr, "\tmask values below ",self.mask," with black"
        if self.picture == "-":
    ...
    def _createCalFiles(self):
        ...
        f_pc0 = open(pc0, 'w')
        f_pc0.write(TEMPLATE_PC0 % (self.scale, self.mult, self.ndivs, self.zerooff, self.minimum, self.redv, self.grnv, self.bluv))
        if self.docont == 'b':
    ...
    def doFalsecolor(self, keeptmp=False):
        ...
        print >> sys.stderr, "\tcreate part images, combine and store image data"
        if self.error != "":
        ...
        try:
            if not self._input:
                self.readImageData()
                print >> sys.stderr, "\tself.data = ", len(self.data), " bytes"
            if self._input:
                print >> sys.stderr, "\tself.data = ", len(self.data), " bytes"
                self.falsecolor()
            ...
            if self.data:
                ...
                print >> sys.stderr, "\tcmd after legend: ", cmd
                if cmd != '':
                    print >> sys.stderr, "before legend Pipe"
                    self.data = self._popenPipeCmd(cmd, self.data)
                    print >> sys.stderr, "after legend Pipe"
                    self.log("self.data=%d bytes" % len(self.data))
                else:
                    print >> sys.stderr, "\tNo legend"
    ...
    def falsecolor(self, data=""):
        ...
        print >> sys.stderr, "\tconvert image data to falsecolor image data"
        if data == "":
            data = self._input
        ...
        print >> sys.stderr, "\tcmd falsecolor: ", cmd
    ...
    def readImageData(self, picture=''):
        ...
        print >> sys.stderr, "\tload image data into self._input"
        try:
            if picture != '':
    ...
    def resetDefaults(self):
        ...
        self.minimum = 0.0      # half step legend offset from zero
	...
	if "-h" in args:
        ...
        try:
		    ...
		    self.redv='mno_red(vin(v))'
		    self.grnv='mno_grn(vin(v))'
		    self.bluv='mno_blu(vin(v))'
		    # self.redv='old_red(vin(v))'
		    # self.grnv='old_grn(vin(v))'
		    # self.bluv='old_blu(vin(v))'
                    ...
                    self.redv ='hot_red(vin(v))'
                    self.grnv ='hot_grn(vin(v))'
                    self.bluv ='hot_blu(vin(v))'
                    
                elif args[0] == '-mono_b':
		    self.redv='mono_b_red(vin(v))'
		    self.grnv='mono_b_grn(vin(v))'
		    self.bluv='mono_b_blu(vin(v))'

                elif args[0] == '-mono_r':
		    self.redv='mono_r_red(vin(v))'
		    self.grnv='mono_r_grn(vin(v))'
		    self.bluv='mono_r_blu(vin(v))'

                elif args[0] == '-mono_ry':
		    self.redv='mono_ry_red(vin(v))'
		    self.grnv='mono_ry_grn(vin(v))'
		    self.bluv='mono_ry_blu(vin(v))'
                ...
                ## contour line and band option
                    ...
                    option = args.pop(0)
                    self.minimum = float(args[0])
                    self.legend.minimum = float(args[0])
